...
// --- SVG Icons & Helper Functions (Copied from your file) ---
// (We need these here because AppointmentCard uses them)
...
// --- Appointment Card Component (Copied from your file) ---
    ...
    return (
        <div className={`flex flex-col h-full p-6 rounded-2xl shadow-xl border ${isDark ? 'bg-gradient-to-br from-[#1E293B] to-[#0F172A] border-gray-700' : 'bg-white border-gray-200'}`}>
            ...
            <div className="mt-auto">
                <button onClick={handleViewDetails} className="w-full py-3 px-6 rounded-xl text-lg font-semibold text-white bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 focus:outline-none focus:ring-4 focus:ring-green-500 focus:ring-opacity-50 transition-all duration-300 ease-in-out shadow-lg">View Patient History</button>
            </div>
        </div>
    );
};
// --- END Appointment Card Component ---


// --- Main Page Component (FOR COMPLETED RECORDS) ---
const CompletedAppointmentsPage = () => {
    ...
    return (
        <div className={`min-h-screen p-4 sm:p-6 lg:p-8 font-sans transition-colors duration-300 ${theme === 'dark' ? 'bg-gradient-to-br from-[#0F172A] to-[#1E293B]' : 'bg-gray-100'}`}>
            <div className="max-w-7xl mx-auto">
                <header className="mb-10 flex justify-between items-center">
                    <div className="text-left">
                        {/* Changed Title */}
                        <h1 className={`text-5xl font-extrabold mb-3 tracking-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Completed Records</h1>
                        <p className={`text-xl ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>View all past appointment history.</p>
                    </div>
                    <button onClick={toggleTheme} className={`p-2 rounded-full transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${theme === 'dark' ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-white text-gray-800 hover:bg-gray-200'}`} aria-label="Toggle theme">{theme === "dark" ? <FiSun size={20} /> : <FaMoon size={20} />}</button>
                </header>

                <main>
                    {loading && <p className={`text-center text-xl ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Loading records...</p>}
                    {error && <p className={`text-center text-xl text-red-500`}>Error: {error}</p>}
                    
                    {!loading && !error && (
                        <>
                            {appointments.length > 0 ? (
                                // --- CHANGED: Replaced Swiper with a responsive grid ---
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {appointments.map(app => (
                                        <AppointmentCard key={app._id} appointment={app} theme={theme} />
                                    ))}
                                </div>
                            ) : (
                                <p className={`text-center text-xl ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>No completed appointments found.</p>
                            )}
                        </>
                    )}
                </main>
